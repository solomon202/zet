...
//Аннотация @Autowired отмечает конструктор, поле или метод как требующий автозаполнения
	...
	//интерфейс предназначен для добавления атрибутов в модель передачи из html в модель
	public String viewHomePage(Model model) {
		// вставить в атребуты данные с помощью модели
	// сортировка по отребутом обьекта  
		//переход поле сортировки в изночальное состояне ASC – (по умолчанию, необязательный). Сортирует набор в порядке возрастания или по алфавиту.
	...
	public String wer(Model model) {
		//метод модели в параметрах вставляем атребут куда вставвит  и метод сервис запус этого метода в нутри которого исполнительный код
		//метод сервиса 
  ...
  return "inde";
	...
	public String showNewEmployeeForm(Model model) {
		//модель  возвращать не полностью entity, а только что мы хотим вернуть или как то обработать, без бизнес логики.
	...
	//@ModelAttribute("employee")вкачестве аргумента метода он указывает, что аргумент должен быть извлечен из модели
	...
	public String showFormForUpdate(@PathVariable ( value = "id") long id, Model model) {
		...
		// установите employee в качестве атрибута модели для предварительного заполнения формы
		model.addAttribute("employee", employee);
